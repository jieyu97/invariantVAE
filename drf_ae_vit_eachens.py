...
from model.ae_vit_new3 import AEens
...
def main():
    ...
    for VAR in ['t2m','u10','v10']:
        ...
        f = open('./'+model_name+'_d'+str(dim_latent)+'_eachens_'+VAR+'.txt','w')
        
        print("Model: ViT AE with patch size 9 (81-->9); no additional orog-lsm input.", file=f)
        ...
        model = AEens(latent_dim=N_LATENT)
        model.to(device)
