...
def main():
    ...
    #for VAR in ['z500', 't2m', 'u10', 'v10']:
        for N_LATENT in [2, 4, 8, 16, 32]:
            ...
            train, val, test, _ = build_ens_allvar_dataset(var=VAR, reorder=ORDER)
            ...
            recons = recons_test.reshape((-1, 50, 81, 81), order=ORDER)
            raw = data_test.reshape((-1, 50, 81, 81), order=ORDER)
            
            latent_test = latent_test.reshape((-1, 50, N_LATENT), order=ORDER)
            # shape (n, 50, 32)
            latent_train = latent_train.reshape((-1, 50, N_LATENT), order=ORDER)
            latent_val = latent_val.reshape((-1, 50, N_LATENT), order=ORDER)
